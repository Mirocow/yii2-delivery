...
* https://www.dpd.ru/dpd/integration/integration.do2
* https://ru.wordpress.org/plugins/woo-dpd
* https://documenter.getpostman.com/view/7354859/SzezaWVs?version=latest
* https://bitbucket.org/DPDinRussia/dpd.sdk
...
Создаем файл city.php (исполняющий файл, можно назвать как угодно.).
...
foreach($mass AS $key => $key1){
	foreach($key1 AS $cityid => $city){
	 if (in_array($idcity,$city)) {$id = $city['cityId']; return $id;}// если мы находим этот город в масиве (который мы искали) мы заносим его в переменную $ID и возвращаем наш ответ.
	}
} 
}
...
foreach ($uchetki as $key => $val) {
    if ($val[0] == $uchet){ $result = $key; break; }
}
